...
def generate_executive_summary_recommendation(top_appliance: str, 
    ...
    """
    summary = (
        f"Analytical results identify **{top_appliance}** as the primary energy load, contributing **{top_share:.2f}%** "
        f"of total estimated appliance consumption. "
        f"Seasonal analysis shows dry season consumption is **{dry_vs_wet_diff_pct:.2f}% higher**, indicating peak demand during warmer months. "
        f"The evaluated 15% Moderate Conservation Scenario establishes an optimized target of **{optimized_target_kwh:,.2f} kWh/month**, "
        f"delivering an estimated annual reduction of **4,120.32 kWh**, **₱45,323.52**, and avoiding **2,884.22 kg CO₂e**."
    )
    return summary
